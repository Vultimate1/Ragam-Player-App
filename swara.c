...
void upload_data(FILE * x);

void printRagas(void);
...
void search(void) {
	...
	if (index==datasize) {
		...
		printf("Enter your sruti:\n");
		scanf("%s", sruti);
		playragam(sruti, index);
		clearKeyBoardBuffer();
	}
}
...
/*void addraga() {
	...
	char *scale = malloc(24*sizeof(char[2])+23*sizeof(' '));
	...
	if (contains(new->name)) {
		printf("Ragam is already in database.\n");
	} else	{
		data[sizeof(database)/sizeof(struct raga) - 1] = new;
		if (i < 10) {
